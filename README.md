...
## Demo Resources
[![Slides](https://img.shields.io/badge/Slides-View%20Presentation-blue)](https://drive.google.com/file/d/1l70RBB-kpkhAd9cIEGwsQ03ellFhz-2s/view?usp=sharing)
...
- **142 tests, warning-free build.** 135 unit/invariant (incl. fuzzing) + **7 live-fork tests** that replay all three legs against their real chains' state: legs ① + ③ on a Unichain Sepolia fork (the live hook + receiver), and leg ② on a HyperEVM **mainnet** fork (the real `LambdaHedger` firing a correct CoreWriter order, asserted byte-for-byte). Run: `forge test`; for the forks see [`FORK_TESTING.md`](./FORK_TESTING.md).
- **Integrations cross-checked + testing methodology + honest limits.** Every partner integration is verified field-by-field against its authoritative source; the suite uses **every technique UHI's hook-testing lesson teaches** (unit, fuzz with `bound()` + `vm.assume()`, fork-in-test) plus invariant suites; and the mainnet-only behaviours that testnet can't exercise are listed plainly, all in [`VERIFICATION.md`](./VERIFICATION.md) (§⑤-⑥).
...
- **Mainnet promotion is a documented checklist, not a rewrite.** The Reactive Network team confirmed a setup proven on a supported testnet carries to HyperEVM mainnet unchanged. Promotion means: point the Reactive leg at HyperEVM mainnet (`DESTINATION_CHAIN_ID=999`), set `invertedPair=true` for a USDC/WETH pool (USDC sorts below WETH so the volatile asset is token1), use the calibrated price scales (`szScaleWad=1e8`, `pxScaleWad=1e20`), and call `fundMargin()` to seed perp margin after bridging. No contract rewrite. Full runbook in [Path to mainnet](#path-to-mainnet).
...
| Metric | Value |
|---|---|
| Foundry tests passing | **142** (135 unit/invariant + 7 live-fork) |
| Test suites | **18** |
...
where **σ** (sigma) is the asset's volatility, meaning how much its price swings around. The more an asset moves, the more the LP loses.
...
| Funding income from the short | n/a | **+10-15% / yr** *(collects the LVR back)* |
| Gamma + rebalancing cost | n/a | −0.5% *(`τ`-banded)* |
...
| **Reactive Network** | A Reactive Smart Contract (`AbstractReactive`) subscribes to the hook's `HedgeRequested` event on Unichain and fires a cross-chain callback (`AbstractCallback`) to the hedger, with no off-chain bot. Nonce-ordered replay protection on both legs. **Verified live** on Reactive Lasna → Unichain Sepolia. | [`contracts/src/LambdaReactive.sol`](./contracts/src/LambdaReactive.sol), [Live on testnet](#live-on-testnet) |
...
|---|---|
| **No `v4-periphery` dependency** | `BaseHook` lives in `v4-periphery`; Lambda doesn't vendor it. Direct `IHooks` needs only `v4-core`, giving a smaller dependency/trust/version surface (and `BaseHook` has changed across periphery versions). |
...
| **Slightly less gas** | No extra virtual-dispatch layer (`BaseHook` routes `beforeSwap` → internal `_beforeSwap`); Lambda calls its logic directly, which is meaningful since this runs on every swap. |
...
- **Uniswap's own governance forum has already validated this strategy.** An independent team — with no connection to Lambda — posted a public RFC directly on the Uniswap governance forum proposing the same solution: a v4 hook combining dynamic fees with a perpetual hedge to cancel LP impermanent loss. That RFC lives on Uniswap's own platform: [gov.uniswap.org](https://gov.uniswap.org/t/rfc-il-hedge-hook-automated-impermanent-loss-protection-for-uniswap-v4-lps/26059). This means the Uniswap community itself — independently, on its own forum — has already said this is the right problem to solve and this is the right approach to take. Lambda has built what that RFC proposed, and gone further: no underwriter pool, no premium mechanism, no off-chain bot — the loss is cancelled at source by the LVR ⇋ funding identity, automatically and cross-chain. Awarding Lambda is not a bet; it is backing what the Uniswap community has already asked for, built and verified on-chain.
- **Lambda is the only approach in this space that holds under stress.** Two other design strategies for LP protection have been analysed in depth: algebraic elimination (proved impossible for all initial pool states by [Voronin et al., arXiv:2604.28017](https://arxiv.org/abs/2604.28017)) and tranche redistribution (zero-sum, collapses when volatility is highest). Both fail precisely when LPs need protection most. Lambda cancels the loss at source — the only approach whose protection scales with σ² rather than breaking under it. The governance forum RFC, the academic proofs, and the live on-chain deployment all point in the same direction. Supporting Lambda is supporting the solution the evidence converges on.
...
| poolId | (all chains) | `0x92fcee81621f08f93eb2e42cbb5e42d969459a5e41cda459b329cbbd0ec4373b` |
...
It is simply not *deployed* on testnet because of the routing gap above. On mainnet the same verified loop targets the real `LambdaHedger` on HyperEVM via the documented checklist in [Path to mainnet](#path-to-mainnet).

---

## Judge runbook (5-minute clickthrough)

A frictionless way to verify the loop end-to-end yourself, with no setup beyond a browser and a wallet:
...
5. **Verify the cross-chain delivery without the UI.** Anyone with `cast` can confirm the receiver state directly:
   ...
   cast call 0x36C7AA315e4Cd8aB7E8CADfbD5B10A3Fb03c2E0C \
     ...
     --rpc-url https://sepolia.unichain.org
   ```
   The returned tuple `(lastNonce, count, targetSize, sqrtPriceX96)` is the exact hedge Lambda computed, e.g. `(3, 1, 39.7e18, …)` meaning the last hook nonce applied, the number of hedges received, and `targetSize = 0.65 × delta`, delivered cross-chain by Reactive, with no off-chain bot.
...
**Want to run the tests?** `forge test` from the repo root. **135 unit/invariant tests pass** in ~1.5 s. To replay all three legs against their real chains on forks (7 green, no gas; see [`FORK_TESTING.md`](./FORK_TESTING.md)):
```bash
...
Lambda is submitted on **testnet**, where the live pieces run against real infrastructure: a real v4 hook on Unichain Sepolia and real Reactive automation on Lasna, and the Hyperliquid perp leg is proven against **real HyperEVM mainnet state on a fork** (the real `LambdaHedger` firing a correct CoreWriter order, asserted byte-for-byte; the CoreWriter precompile itself verified live on-chain). The one limitation is external: Reactive's testnet does not route callbacks to HyperEVM testnet, so the cross-chain hedge is demonstrated as **two proven halves** (the automatic Unichain → Reactive callback, live on testnet, and the real CoreWriter perp, fork-proven on HyperEVM). The Reactive Network team confirmed this routing limitation directly and confirmed that the testnet-proven setup will carry over to HyperEVM mainnet without change. On mainnet the two halves become **one automatic loop**, and promotion is deliberately a *configuration* change, not a rewrite.
...
forge test               # 135 unit/invariant tests pass (the 7 fork tests self-skip offline)
```
...
- **[Foundry](https://book.getfoundry.sh)**: build, fuzzing, property tests, and live-fork integration across two chains (142 passing)
